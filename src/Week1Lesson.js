...
import { Home, Play, Volume2, CheckCircle, XCircle } from 'lucide-react';
...
const Week1Lesson = () => {
  ...
  const mistakes = [
    {
      title: "실수 #1: 모르는데 'Yes'",
      problem: "못 알아듣고 Yes → 큰일남",
      solution: "Sorry, can you repeat that?"
    },
    {
      title: "실수 #2: 혼자 조용히 있기",
      problem: "영어 못해서 안 섞임 → 오해받음",
      solution: "간단하게 반응: Nice! / Good job!"
    },
    {
      title: "실수 #3: 부상 숨기기",
      problem: "작은 부상 → 큰 부상",
      solution: "Coach, my arm hurts."
    },
    {
      title: "실수 #4: 문자 답장 안하기",
      problem: "무시당했나 생각함",
      solution: "1시간 내 답장하기"
    }
  ];
    ...
    <div className="bg-gray-50 min-h-screen p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-blue-800 text-white p-8 rounded-t-2xl">
          <h1 className="text-5xl font-bold mb-2">⚾ Week 1</h1>
          <h2 className="text-3xl font-semibold">팀 첫날 생존 영어</h2>
          <p className="text-xl mt-4">자기소개 & 팀원과의 첫 대화</p>
        </div>

        <div className="bg-white shadow-2xl rounded-b-2xl">
          
          {/* Part 1 */}
          <section className="p-8 border-b-4 border-gray-200">
            <h3 className="text-3xl font-bold text-blue-700 mb-6">🤝 Part 1: 첫 만남 인사</h3>
            
            <div className="grid md:grid-cols-2 gap-6 mb-6">
              <div className="bg-green-50 p-6 rounded-xl">
                <h4 className="text-2xl font-bold text-green-700 mb-4">✅ 코치/스태프</h4>
                {greetings.formal.map((item, i) => (
                  <div key={i} className="bg-white p-3 rounded mb-3">
                    <p className="font-bold">{item.eng}</p>
                    <p className="text-sm text-gray-600">{item.kor}</p>
                  </div>
                ))}
              </div>
              <div className="bg-blue-50 p-6 rounded-xl">
                <h4 className="text-2xl font-bold text-blue-700 mb-4">✅ 팀원들</h4>
          ...
          {/* Part 2 */}
          <section className="p-8 border-b-4 border-gray-200">
            <h3 className="text-3xl font-bold text-blue-700 mb-6">🎤 Part 2: 자기소개 템플릿</h3>
            
            <div className="bg-purple-100 p-6 rounded-xl mb-6">
              <h4 className="text-xl font-bold mb-3">📝 30초 버전</h4>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div className="bg-blue-50 p-4 rounded-xl">
                <h4 className="font-bold mb-2">투수 예시</h4>
              ...
              <div className="bg-green-50 p-4 rounded-xl">
                <h4 className="font-bold mb-2">내야수 예시</h4>
                <div className="bg-white p-3 rounded text-sm space-y-1">
          ...
          {/* Part 3 */}
          <section className="p-8 border-b-4 border-gray-200">
            <h3 className="text-3xl font-bold text-blue-700 mb-6">❓ Part 3: 자주 묻는 질문 TOP 8</h3>
            
            <div className="space-y-4">
              {questions.map((item, i) => (
                <div key={i} className="bg-blue-50 p-4 rounded-xl">
                  <p className="text-xl font-bold text-blue-900 mb-2">Q{i+1}: "{item.q}"</p>
                  <div className="bg-white p-3 rounded">
                    <p className="text-green-700 font-semibold">✅ {item.a}</p>
                    <p className="text-sm text-orange-600 mt-1">💡 {item.tip}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Part 4 */}
          <section className="p-8 border-b-4 border-gray-200">
            <h3 className="text-3xl font-bold text-blue-700 mb-6">💬 Part 4: 리액션 표현</h3>
            
            <div className="grid md:grid-cols-4 gap-3">
              {reactions.map((item, i) => (
                <div key={i} className="bg-white p-3 rounded-xl shadow border-2 border-green-200">
                  <p className="text-xl font-bold text-green-700">"{item.word}"</p>
          ...
          {/* Part 5 */}
          <section className="p-8 border-b-4 border-gray-200">
            <h3 className="text-3xl font-bold text-blue-700 mb-6">🎵 Part 5: 발음 주의</h3>
            
            <div className="space-y-3">
              {pronunciation.map((item, i) => (
                <div key={i} className="bg-red-50 p-4 rounded-xl">
                  <p className="text-2xl font-bold text-red-700 mb-2">{item.word}</p>
                  <div className="grid md:grid-cols-2 gap-3">
                    <div className="bg-red-100 p-2 rounded">{item.wrong}</div>
                    <div className="bg-green-100 p-2 rounded">{item.right}</div>
                  </div>
          ...
          {/* Part 6 */}
          <section className="p-8 border-b-4 border-gray-200">
            <h3 className="text-3xl font-bold text-blue-700 mb-6">⚠️ Part 6: 자주 하는 실수</h3>
            ...
            <div className="space-y-4">
              {mistakes.map((item, i) => (
                <div key={i} className="bg-white p-4 rounded-xl border-2 border-gray-200">
                  ...
                  <p className="text-red-700 bg-red-50 p-2 rounded mb-2">문제: {item.problem}</p>
                  <p className="text-green-700 bg-green-50 p-2 rounded">✅ 해결: {item.solution}</p>
                </div>
              ))}
            </div>
          </section>
          ...
          {/* Part 7 */}
          <section className="p-8 border-b-4 border-gray-200">
            <h3 className="text-3xl font-bold text-blue-700 mb-6">🗣️ Part 7: 발음 드릴 (매일 5분)</h3>
          ...
          <section className="p-8">
            <h3 className="text-3xl font-bold text-blue-700 mb-6">📚 이번 주 숙제</h3>
            
            <div className="bg-orange-100 p-6 rounded-xl">
              <h4 className="text-2xl font-bold mb-4">✍️ 필수 과제</h4>
        ...
        <div className="bg-gray-800 text-white p-6 rounded-b-2xl flex justify-between items-center">
          <Link to="/" className="text-blue-300 hover:text-blue-100">← 메인</Link>
          <p className="text-xl">⚾ Week 2 준비중</p>
          <Link to="/week1-vocab" className="text-purple-300 hover:text-purple-100">단어장 →</Link>
        </div>
